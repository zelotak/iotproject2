<template>
    <div class="tab-mqtt">
      <!-- Score global -->
      <div class="header">
        <h1>Résultats de l’audit MQTT</h1>
        <div class="score-box">
  ...
  export default {
    ...
    computed: {
      mqttTests() {
        return (this.testResults.mqtt || []).filter(t => 'test' in t)
      },
      score() {
        const scoreEntry = (this.testResults.mqtt || []).find(t => 'score' in t)
        return scoreEntry ? scoreEntry.score : 0
      },
      explanations() {
        const mqttExplanations = explanationsJSON['MQTT'] || []
        return Object.fromEntries(mqttExplanations.map(e => [e.test, e]))
      }
    }
  }
  </script>
