<template>
    <div class="tab-coap">
      <!-- Score global -->
      <div class="header">
        <h1>Résultats de l’audit COAP</h1>
        <div class="score-box">
  ...
  export default {
    ...
    computed: {
      coapTests() {
        return (this.testResults.coap || []).filter(t => 'test' in t)
      },
      score() {
        const scoreEntry = (this.testResults.coap || []).find(t => 'score' in t)
        return scoreEntry ? scoreEntry.score : 0
      },
      explanations() {
        const coapExplanations = explanationsJSON['COAP'] || []
        return Object.fromEntries(coapExplanations.map(e => [e.test, e]))
      }
    }
  }
  </script>
