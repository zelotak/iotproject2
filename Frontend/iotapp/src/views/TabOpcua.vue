<template>
    <div class="tab-opcua">
      <!-- Score global -->
      <div class="header">
        <h1>Résultats de l’audit OPCUA</h1>
        <div class="score-box">
      ...
      <div class="test-grid">
        <div
          v-for="(test, index) in opcuaTests"
          :key="index"
          class="test-card"
  ...
  export default {
    ...
    computed: {
      opcuaTests() {
        return (this.testResults.opcua || []).filter(t => 'test' in t)
      },
      score() {
        const scoreEntry = (this.testResults.opcua || []).find(t => 'score' in t)
        return scoreEntry ? scoreEntry.score : 0
      },
      explanations() {
        const opcuaExplanations = explanationsJSON['OPCUA'] || []
        return Object.fromEntries(opcuaExplanations.map(e => [e.test, e]))
      }
    }
  }
  </script>
