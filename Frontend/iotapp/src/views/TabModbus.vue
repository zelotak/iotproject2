<template>
    <div class="tab-modbus">
      <!-- Score global -->
      <div class="header">
        <h1>Résultats de l’audit MODBUS</h1>
        <div class="score-box">
      ...
      <div class="test-grid">
        <div
          v-for="(test, index) in modbusTests"
          :key="index"
          class="test-card"
  ...
  export default {
    ...
    computed: {
      modbusTests() {
        return (this.testResults.modbus || []).filter(t => 'test' in t)
      },
      score() {
        const scoreEntry = (this.testResults.modbus || []).find(t => 'score' in t)
        return scoreEntry ? scoreEntry.score : 0
      },
      explanations() {
        const modbusExplanations = explanationsJSON['MODBUS'] || []
        return Object.fromEntries(modbusExplanations.map(e => [e.test, e]))
      }
    }
  }
  </script>
